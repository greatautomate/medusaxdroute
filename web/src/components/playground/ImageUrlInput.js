    ...
    <div className={disabled ? 'opacity-50' : ''}>
      ...
      {!imageEnabled ? (
        <Typography.Text className="text-xs text-gray-500 mb-2 block">
          {disabled ? t('图片功能在自定义请求体模式下不可用') : t('启用后可添加图片URL进行多模态对话')}
        </Typography.Text>
      ) : imageUrls.length === 0 ? (
        <Typography.Text className="text-xs text-gray-500 mb-2 block">
          {disabled ? t('图片功能在自定义请求体模式下不可用') : t('点击 + 按钮添加图片URL进行多模态对话')}
        </Typography.Text>
      ) : (
        <Typography.Text className="text-xs text-gray-500 mb-2 block">
          {t('已添加 {{count}} 张图片', { count: imageUrls.length })}{disabled ? t(' (自定义模式下不可用)') : ''}
        </Typography.Text>
      )}
