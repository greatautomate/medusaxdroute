...
function type2secretPrompt(type, t) {
  // Use translation function for all prompts
  switch (type) {
    case 15:
    ...
    case 50:
      return t('按照如下格式输入: AccessKey|SecretKey');
    case 51:
      return t('按照如下格式输入: Access Key ID|Secret Access Key');
    default:
...
const EditChannel = (props) => {
  ...
  const handleInputChange = (name, value) => {
    ...
    if (name === 'base_url' && value.endsWith('/v1')) {
      Modal.confirm({
        title: t('警告'),
        content: t('不需要在末尾加/v1，MedusaXD API会自动处理，添加后可能导致请求失败，是否继续？'),
        onOk: () => {
  ...
  const fetchUpstreamModelList = async (name) => {
    ...
    if (isEdit) {
      // If in edit mode, use existing channel id to get model list
      ...
      // If in create mode, get model list through backend proxy
      if (!inputs?.['key']) {
        showError(t('请填写密钥'));
        err = true;
      } else {
